...
// Dashboard Page
function Dashboard() {
  const [stats, setStats] = useState({ total_active_donors: 0, total_communities: 0, total_items_in_stock: 0, pending_requests: 0 });
  const [recentDonors, setRecentDonors] = useState([]);
  const [completedRequestsData, setCompletedRequestsData] = useState([]);
  const [pendingRequestsData, setPendingRequestsData] = useState([]);
  const [recentItems, setRecentItems] = useState([]);
  const [timeReports, setTimeReports] = useState({ last_7_days: { items_donated: 0, items_distributed: 0, requests_fulfilled: 0, requests_pending: 0 }, last_30_days: { items_donated: 0, items_distributed: 0, requests_fulfilled: 0, requests_pending: 0 }, last_90_days: { items_donated: 0, items_distributed: 0, requests_fulfilled: 0, requests_pending: 0 } });
  ...
  const fetchDashboard = () => {
    fetch('http://localhost:5000/api/admin/dashboard')
      ...
      .then(data => {
        ...
        if (data.recentDonors) setRecentDonors(data.recentDonors);
        if (data.completedRequests) setCompletedRequestsData(data.completedRequests);
        if (data.pendingRequests) setPendingRequestsData(data.pendingRequests);
        if (data.recentItems) setRecentItems(data.recentItems);
        if (data.timeReports) setTimeReports(data.timeReports);
        setLoading(false);
      })
      .catch(error => {
  ...
  useEffect(() => {
    // Create items by category chart using real data
        ...
        <div className="card">
          ...
          <div className="card-body">
            ...
            <div style={{ marginBottom: '20px' }}>
              {completedRequestsData && completedRequestsData.length > 0 ? completedRequestsData.map(req => (
                <div key={req.request_id} style={{ padding: '12px', backgroundColor: 'var(--color-bg-3)', borderRadius: '8px', marginBottom: '8px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                    <div>
                      ...
                      <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '4px' }}>Items: {req.items_count} | Completed: {req.completion_date ? new Date(req.completion_date).toLocaleDateString() : 'N/A'}</div>
                    </div>
                    <span className="status-badge completed" style={{ fontSize: '11px' }}>{req.status}</span>
                  </div>
                </div>
              )) : <p style={{ color: '#6b7280', fontSize: '13px' }}>No completed requests yet</p>}
            </div>
            <h3 style={{ fontSize: '14px', fontWeight: '600', marginBottom: '12px', color: 'var(--color-text)' }}>Last 5 Items Donated</h3>
            {recentItems && recentItems.length > 0 ? recentItems.slice(0, 5).map(item => (
              <div key={item.item_id} style={{ fontSize: '13px', padding: '6px 0', borderBottom: '1px solid #e5e7eb' }}>
                {item.item_name} - {item.donor_name}
              </div>
            )) : <p style={{ color: '#6b7280', fontSize: '13px' }}>No items donated yet</p>}
          </div>
        </div>
          ...
          <div className="card-body">
            <div style={{ marginBottom: '16px', padding: '12px', backgroundColor: 'var(--color-bg-1)', borderRadius: '8px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '12px' }}>
                <div>Items Donated: <strong>{timeReports?.last_7_days?.items_donated || 0}</strong></div>
                <div>Items Distributed: <strong>{timeReports?.last_7_days?.items_distributed || 0}</strong></div>
                <div>Requests Fulfilled: <strong>{timeReports?.last_7_days?.requests_fulfilled || 0}</strong></div>
                <div>Requests Pending: <strong>{timeReports?.last_7_days?.requests_pending || 0}</strong></div>
              </div>
            </div>
            <div style={{ marginBottom: '16px', padding: '12px', backgroundColor: 'var(--color-bg-2)', borderRadius: '8px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '12px' }}>
                <div>Items Donated: <strong>{timeReports?.last_30_days?.items_donated || 0}</strong></div>
                <div>Items Distributed: <strong>{timeReports?.last_30_days?.items_distributed || 0}</strong></div>
                <div>Requests Fulfilled: <strong>{timeReports?.last_30_days?.requests_fulfilled || 0}</strong></div>
                <div>Requests Pending: <strong>{timeReports?.last_30_days?.requests_pending || 0}</strong></div>
              </div>
            </div>
            <div style={{ padding: '12px', backgroundColor: 'var(--color-bg-3)', borderRadius: '8px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '12px' }}>
                <div>Items Donated: <strong>{timeReports?.last_90_days?.items_donated || 0}</strong></div>
                <div>Items Distributed: <strong>{timeReports?.last_90_days?.items_distributed || 0}</strong></div>
                <div>Requests Fulfilled: <strong>{timeReports?.last_90_days?.requests_fulfilled || 0}</strong></div>
                <div>Requests Pending: <strong>{timeReports?.last_90_days?.requests_pending || 0}</strong></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="card-body">
          ...
          <div className="table-container">
            <table>
              ...
              <tbody>
                {pendingRequestsData && pendingRequestsData.length > 0 ? pendingRequestsData.map(req => (
...
// Communities Management Page - Shows ONLY verified/registered recipients
function CommunitiesManagement() {
  ...
  // Fetch only verified recipients
  useEffect(() => {
    fetch('http://localhost:5000/api/admin/recipients?status=verified&limit=100')
      .then(response => response.json())
      .then(data => {
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(r => 
        ...
        r.recipient_code?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div>
      <div className="page-header">
        <h1 className="page-title">Registered Recipients</h1>
        <p className="page-subtitle">All verified and registered community organizations that can request resources</p>
      </div>
      ...
      <div className="filters">
        ...
        <div className="filter-group" style={{ marginLeft: 'auto' }}>
          <span style={{ 
            ...
            {recipients.length} Registered Recipients
          </span>
        </div>
      </div>
      ...
      <div className="card">
              ...
              {paginatedRecipients.length > 0 ? (
                paginatedRecipients.map(recipient => (
                  <tr key={recipient.recipient_id}>
                    <td>
                      <span style={{ fontFamily: 'monospace', fontWeight: '600', color: '#166534' }}>
                        {recipient.recipient_code || 'N/A'}
                      </span>
                    </td>
                    <td>{recipient.full_name}</td>
                    <td>{recipient.guardian_name}</td>
                    <td>{recipient.guardian_contact}</td>
                ...
                <tr>
                  <td colSpan="7">
                    <div className="empty-state">
                      <div className="empty-state-icon">🏫</div>
                      <div className="empty-state-text">No registered recipients found</div>
                    </div>
                  </td>
                </tr>
              )}
        ...
        {filteredRecipients.length > 0 && (
          <div className="pagination">
            <div className="pagination-info">
              Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, filteredRecipients.length)} of {filteredRecipients.length} recipients
            </div>
            <div className="pagination-controls">
              <button 
                className="pagination-btn" 
...
function RequestsManagement() {
  ...
  // Pending recipient applications state
  ...
  // Fetch pending recipient applications
  const fetchPendingRecipients = () => {
    fetch('http://localhost:5000/api/admin/recipients?status=pending&limit=50')
      .then(response => response.json())
      .then(data => {
        setPendingRecipients(data.recipients || []);
      })
      .catch(error => console.error('Error fetching recipients:', error));
  };
  ...
  // Handle approve recipient
  const handleApprove = async (recipient) => {
    setActionLoading(true);
    try {
      const response = await fetch(`http://localhost:5000/api/admin/recipients/${recipient.recipient_id}/approve`, {
        method: 'POST',
      ...
      if (response.ok) {
        setApprovalCredentials(data.credentials);
        setSelectedRecipient(recipient);
        setShowPreviewModal(false);
        setShowApprovalModal(true);
        fetchPendingRecipients();
      } else {
        alert(data.message || 'Error approving recipient');
      }
    } catch (error) {
      console.error('Approve error:', error);
      alert('Error approving recipient');
    } finally {
      setActionLoading(false);
    }
  };

  // Handle reject recipient
  const handleReject = async () => {
    ...
    setActionLoading(true);
    try {
      const response = await fetch(`http://localhost:5000/api/admin/recipients/${selectedRecipient.recipient_id}/reject`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ adminId: 1, reason: rejectionReason })
      });
      const data = await response.json();
      if (response.ok) {
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p className="page-subtitle">Manage recipient applications and resource requests</p>
      </div>

      {/* Pending Recipient Applications Section */}
      <div className="card" style={{ marginBottom: '24px' }}>
        <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 className="card-title">Pending Recipient Applications</h2>
          <span style={{ 
            backgroundColor: pendingRecipients.length > 0 ? '#fef3c7' : '#d1fae5',
            color: pendingRecipients.length > 0 ? '#92400e' : '#065f46',
                  ...
                  <tr key={recipient.recipient_id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button 
                          onClick={() => { setSelectedRecipient(recipient); setShowPreviewModal(true); }}
                          style={{ padding: '6px 10px', fontSize: '14px', cursor: 'pointer', backgroundColor: '#f3f4f6', border: '1px solid #d1d5db', borderRadius: '4px' }}
                          title="View Application"
                        >
                          📄
                        </button>
                        <button 
                          onClick={() => handleApprove(recipient)}
                          disabled={actionLoading}
                          style={{ padding: '6px 10px', fontSize: '14px', cursor: 'pointer', backgroundColor: '#d1fae5', border: '1px solid #6ee7b7', borderRadius: '4px', color: '#065f46' }}
                          title="Approve"
                        >
                          ✓
                        </button>
                        <button 
                          onClick={() => { setSelectedRecipient(recipient); setShowRejectModal(true); }}
                          style={{ padding: '6px 10px', fontSize: '14px', cursor: 'pointer', backgroundColor: '#fee2e2', border: '1px solid #fca5a5', borderRadius: '4px', color: '#991b1b' }}
                          title="Reject"
                        >
                          ✗
                        </button>
      ...
      {showPreviewModal && selectedRecipient && (
        <div style={{
          ...
          <div style={{
            backgroundColor: 'white', borderRadius: '12px', padding: '24px', maxWidth: '600px', width: '90%', maxHeight: '80vh', overflow: 'auto'
          }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2 style={{ margin: 0 }}>Application Details</h2>
              <button onClick={() => setShowPreviewModal(false)} style={{ border: 'none', background: 'none', fontSize: '24px', cursor: 'pointer' }}>×</button>
            </div>
            
            <div style={{ marginBottom: '16px' }}>
              <h3 style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>Organization Information</h3>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', backgroundColor: '#f9fafb', padding: '16px', borderRadius: '8px' }}>
                <div><strong>Organization:</strong> {selectedRecipient.full_name}</div>
                <div><strong>Contact Person:</strong> {selectedRecipient.guardian_name}</div>
                <div><strong>Phone:</strong> {selectedRecipient.guardian_contact}</div>
                <div><strong>Applied:</strong> {new Date(selectedRecipient.registration_date).toLocaleDateString()}</div>
              </div>
            </div>
            
            <div style={{ marginBottom: '16px' }}>
              <h3 style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>Address</h3>
            ...
            <div style={{ marginBottom: '20px' }}>
              <h3 style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>Application Letter</h3>
              <div style={{ 
                backgroundColor: '#fffbeb', padding: '16px', borderRadius: '8px', 
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
              ...
              <button 
                onClick={() => { setShowPreviewModal(false); setShowRejectModal(true); }}
                style={{ padding: '10px 20px', borderRadius: '6px', border: 'none', backgroundColor: '#ef4444', color: 'white', cursor: 'pointer' }}
              >
                ✗ Reject
              </button>
              <button 
                onClick={() => handleApprove(selectedRecipient)}
                disabled={actionLoading}
                style={{ padding: '10px 20px', borderRadius: '6px', border: 'none', backgroundColor: '#22c55e', color: 'white', cursor: 'pointer' }}
              >
                ✓ Approve
              </button>
            </div>
      ...
      {showApprovalModal && approvalCredentials && (
        <div style={{
          position: 'fixed', top: 0, left: 0, right: 0, bottom: 0,
          backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000
        }}>
          <div style={{
            backgroundColor: 'white', borderRadius: '12px', padding: '32px', maxWidth: '400px', width: '90%', textAlign: 'center'
          }}>
            <div style={{ fontSize: '48px', marginBottom: '16px' }}>✅</div>
            <h2 style={{ margin: '0 0 16px 0', color: '#22c55e' }}>Application Approved!</h2>
            <p style={{ color: '#6b7280', marginBottom: '16px' }}>Recipient Code Generated:</p>
            <div style={{ 
              backgroundColor: '#f0fdf4', padding: '16px', borderRadius: '8px', 
              border: '2px dashed #22c55e', marginBottom: '20px'
            }}>
              <span style={{ fontSize: '20px', fontWeight: '700', fontFamily: 'monospace', color: '#166534' }}>
                {approvalCredentials.recipientCode}
              </span>
            </div>
            <p style={{ color: '#6b7280', fontSize: '14px', marginBottom: '20px' }}>
              Please share this code with the recipient. They can use this to login to the portal.
            </p>
            <button 
              onClick={() => { setShowApprovalModal(false); setApprovalCredentials(null); setSelectedRecipient(null); }}
              style={{ padding: '12px 32px', borderRadius: '6px', border: 'none', backgroundColor: '#22c55e', color: 'white', cursor: 'pointer', fontSize: '16px' }}
            >
              Done
            </button>
          </div>
      ...
      {/* Reject Modal */}
      {showRejectModal && selectedRecipient && (
        <div style={{
          position: 'fixed', top: 0, left: 0, right: 0, bottom: 0,
          backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000
        }} onClick={() => setShowRejectModal(false)}>
          <div style={{
            backgroundColor: 'white', borderRadius: '12px', padding: '24px', maxWidth: '450px', width: '90%'
          }} onClick={e => e.stopPropagation()}>
            <h2 style={{ margin: '0 0 8px 0', color: '#ef4444' }}>Reject Application</h2>
            <p style={{ color: '#6b7280', marginBottom: '16px' }}>
              Rejecting application for: <strong>{selectedRecipient.full_name}</strong>
            </p>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <textarea 
                ...
                style={{ 
                  ...
                  border: '1px solid #d1d5db', resize: 'vertical', fontFamily: 'inherit'
                }}
              />
            </div>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
              ...
              <button 
                ...
                style={{ 
                  ...
                  backgroundColor: actionLoading || !rejectionReason.trim() ? '#fca5a5' : '#ef4444', 
                  color: 'white', cursor: actionLoading || !rejectionReason.trim() ? 'not-allowed' : 'pointer' 
                }}
              >
      ...
      <h2 style={{ marginBottom: '16px' }}>Item Requests</h2>
      <div className="filters">
        <div className="filter-group">
